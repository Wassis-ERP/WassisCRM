...
export function FinanceStep({ draft, lookups, update }: StepProps) {
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <Field label="Forma de pagamento"><select value={draft.paymentMethod} onChange={(e) => update({ paymentMethod: e.target.value })} className={controlClass}><option value="BOLETO">Boleto</option><option value="CARTAO">Cartão</option><option value="CARTAO_CREDITO">Cartão de crédito</option><option value="DEBITO">Débito em conta</option><option value="DEBITO_CONTA">Débito em conta (cotação)</option><option value="PIX">Pix</option></select></Field>
