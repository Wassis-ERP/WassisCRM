      ...
      <div className="space-y-6">
        <DetailCard title="Ocorrência e aviso" icon={AlertTriangle} action={<EditActions isSaving={isSaving} onCancel={onCancel} onSave={submit} />}>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Responsável"><select value={patch.responsavel_id ?? ''} onChange={(event) => setText('responsavel_id', event.target.value)} className={inputClass}><option value="">Sem responsável</option>{responsaveis.map((profile) => <option key={profile.id} value={profile.id}>{profile.nome_completo ?? profile.email ?? profile.id}</option>)}</select></Field>
