...
import { useEmissao, useUpdateEmissao } from '../hooks/useEmissoes';
...
/**
 * Detalhe de uma emissao. Permite edicao dos campos core + metadata JSONB
 * (numero_proposta, numero_apolice, forma_pagamento, quantidade_parcelas).
 */
export default function EmissaoDetalhePage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const emissaoId = String(id ?? '');

  const detail = useEmissao(emissaoId);
  const update = useUpdateEmissao();
  ...
  const cardForConclude: KanbanCard | null = rawRow
    ? {
        ...
        title: oportunidade?.nome ?? 'Emissao',
  ...
  if (detail.isLoading) {
    return (
      <div className="animate-fade-in flex items-center justify-center py-24">
        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Carregando emissao...</p>
      </div>
    );
  }
  ...
  if (detail.isError || !rawRow) {
    return (
      <div className="animate-fade-in flex flex-col items-center justify-center py-24 gap-4">
        <p className="text-rose-500 font-bold uppercase tracking-widest text-xs">Emissao nao encontrada</p>
  ...
  const clienteNome = segurado?.nome ?? oportunidade?.nome ?? 'Emissao';
  const email = segurado?.email ?? '';
  const telefone = segurado?.telefone ?? '';
  const criadoPor = responsavel?.full_name ?? '-';
  const shortId = emissaoId.slice(0, 8).toUpperCase();
  const numeroApolice = metaFields.numero_apolice ? String(metaFields.numero_apolice) : null;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-black tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                  <FileText size={18} className="text-primary" /> #{shortId}
                  <span className="text-secondary text-base font-medium">| {clienteNome}</span>
                </h1>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                  Emitido por {criadoPor}{numeroApolice ? ` - Apolice ${numeroApolice}` : ''}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {isConcluded && (
                <span className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest ${
                  ...
                  {(rawRow.status as CardStatus) === 'won' ? 'Emitida' : 'Recusada'}
                </span>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12 animate-fade-in">
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-6">Dados da Emissao</h3>
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <textarea
              ...
              placeholder="Anotacoes sobre a emissao..."
            />
          </div>
        </div>
