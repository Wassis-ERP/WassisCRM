...
describe('backendApi', () => {
  ...
  it('faz logout no servidor com CSRF e limpa a sessão em memória', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0]).toBe('https://api.test/api/identity/csrf')
    expect(fetchMock.mock.calls[3]?.[0]).toBe('https://api.test/api/identity/logout')
    expect((fetchMock.mock.calls[3]?.[1] as RequestInit).credentials).toBe('include')
  })

  it('renova CSRF depois do login antes de salvar ou sair', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ token: 'anonymous-csrf' }) })
      .mockResolvedValueOnce(loginResponse())
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ token: 'authenticated-csrf' }) })
      .mockResolvedValueOnce({ ok: true, status: 204 })
    vi.stubGlobal('fetch', fetchMock)
    const api = await importBackendApi()
    api.setBackendActiveBranch('previous-users-branch')
    await api.loginToBackend('user@test.local', 'fixture')
    await api.requestAuthenticatedBackendJson('/api/segurados/fixture', { method: 'PUT', body: '{}' })
    const [, init] = fetchMock.mock.calls[3] as [string, RequestInit]
    expect(new Headers(init.headers).get('X-CSRF-TOKEN')).toBe('authenticated-csrf')
    expect(new Headers(init.headers).get('X-Office-Branch-Id')).toBeNull()
  })
})
